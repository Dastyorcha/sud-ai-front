...
- `src/shared/lib/query/query-keys.ts` · `queryKeys` · central TanStack Query key factory (cases, participants, hearings, transcript, events, documents, audit, jobs, users) — every feature hook's `queryKey`/`invalidateKeys` should read from here so keys agree.
