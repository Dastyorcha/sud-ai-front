...
- `src/shared/lib/errors/error-map.ts` · `errorMessageKey`, `ErrorCode` · maps a thrown error/`{ code }` (case-insensitive — backend `UPPER_SNAKE` codes normalize to `lower_snake`) to a localized `errors.codes.*` message key; resolve with `t()` and surface via `notify.error`.
