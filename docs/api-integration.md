...
> Temporary public-access mode: product routes, REST calls, and SignalR run
> without login, refresh tokens, role gates, or `Authorization` headers. The
> live case request/response contract no longer contains `judgeId`.
...
- `use-backend-connectivity.ts` / `backend-connectivity-probe.tsx` — the probe
  ...
  never gates rendering; product routes are public.

## HTTP client (`src/shared/lib/http/`)

- `api-client.ts` — single axios instance (`apiClient`). Request interceptor
  sets `Accept: application/json` and a fresh `X-Request-ID` per call
  (`request-id.ts`). It does not attach authentication credentials. Response
...
| ------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| No user/judge list endpoint                                   | Judge identity is represented by case participants; the case API has no `judgeId` field                                                                                                                                                                                                       | `features/case-create/use-create-case.ts`                                                      |
